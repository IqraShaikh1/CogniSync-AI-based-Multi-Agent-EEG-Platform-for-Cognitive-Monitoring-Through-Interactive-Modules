...
// eeg_value = mean EEG amplitude (REQUIRED by scaler)
        const window = samples.slice(-windowSize);
        const eegValue =window.reduce((a, b) => a + b, 0) / window.length;

// signal variance
        const variance =window.reduce((sum, v) => sum + (v - eegValue) ** 2, 0) / window.length;
  ...
  // ✅ REQUIRED 7 FEATURES (EXACT MATCH)
  eeg_value: eegValue,
  alpha_power: bp.alpha,
  beta_power: bp.beta,
  theta_power: bp.theta,
  alpha_beta_ratio: bp.alpha / (bp.beta + 1e-6),
  signal_variance: variance,
  attention_index: bp.beta / (bp.theta + bp.alpha + 1e-6),
});
        ...
        if (apiRes && !apiRes.error) {
          ...
          setStressLevel(apiRes.stress_level || 0);
          setRelaxationLevel(apiRes.relaxation_level || 0);
          ...
          const trackingEntry = {
            ...
            stressLevel: apiRes.stress_level || 0,
            relaxationLevel: apiRes.relaxation_level || 0,
          ...
          console.log("📊 Stress Level:", apiRes.stress_level?.toFixed(1) + "%");
          console.log("📊 Relaxation Level:", apiRes.relaxation_level?.toFixed(1) + "%");
        } else {
          setApiStatus("error");
  ...
  const stopRecording = () => {
    ...
    if (trackingData.length > 0) {
      ...
      const avgStressLevel = trackingData.reduce((sum, entry) => sum + entry.stressLevel, 0) / trackingData.length;
      const avgRelaxationLevel = trackingData.reduce((sum, entry) => sum + entry.relaxationLevel, 0) / trackingData.length;
      ...
      const report = {
        ...
        relaxationLevel: avgRelaxationLevel,
  ...
  const downloadPDF = () => {
    ...
    doc.text("EEG Mental Health Report", pageWidth / 2, y, { align: 'center' });
    y += 15;
    ...
    const stateColor = getStatusColor(finalResult.dominantState || finalResult.prediction);
    if (stateColor === "green") doc.setTextColor(34, 197, 94);
    else if (stateColor === "red") doc.setTextColor(239, 68, 68);
    else if (stateColor === "blue") doc.setTextColor(59, 130, 246);
    else if (stateColor === "yellow") doc.setTextColor(245, 158, 11);
    ...
    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    doc.text("Mental Health Metrics", margin, y);
    y += 8;
    
    doc.setFontSize(11);
    doc.setFont(undefined, 'normal');
    doc.text(`Average Stress Level: ${finalResult.stressLevel.toFixed(1)}%`, margin + 5, y);
    y += 7;
    doc.text(`Average Relaxation Level: ${finalResult.relaxationLevel.toFixed(1)}%`, margin + 5, y);
    y += 15;
    ...
    doc.text("Generated by EEG Mental Health Tracker", pageWidth / 2, y, { align: 'center' });
    
    const filename = `EEG_MentalHealth_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0].replace(/:/g, '-')}.pdf`;
    doc.save(filename);
  };
  ...
  const getStatusColor = (pred) => {
    ...
    if (p.includes("relaxation") || p.includes("reading")) return "green";
    if (p.includes("stress")) return "red";
    if (p.includes("baseline")) return "blue";
    return "blue";
  };
  ...
  return (
    <div className="p-6 bg-gray-950 text-white min-h-screen">
      <div className="flex items-center gap-4 mb-8">
        <h1 className="text-3xl font-bold">🧠 EEG Mental Health Tracker</h1>
      </div>
          ...
          <Card title="📡 Device Setup & Connection">
            ...
            {apiStatus === "error" && (
              <div className="mt-4 bg-red-900/20 border border-red-500/50 rounded-lg p-4 text-red-300 text-sm">
                ...
                <code className="block bg-black/30 p-2 rounded mt-2">python mental_health_app.py</code>
              </div>
            )}
          </Card>
                ...
                <div className="text-center">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-left bg-gray-800/50 rounded-lg p-4">
                    ...
                    <div>
                      ...
                      <div className="text-red-400 text-lg font-bold">{finalResult.stressLevel.toFixed(1)}%</div>
                    </div>
                    <div>
                      <div className="text-gray-400 text-xs">Relaxation Level</div>
                      <div className="text-green-400 text-lg font-bold">{finalResult.relaxationLevel.toFixed(1)}%</div>
                    </div>
                    <div>
              ...
              {prediction ? (
                <div>
                  ...
                  <div className="mt-8 mb-6 grid grid-cols-2 gap-8">
                    <div>
                      <div className="text-sm text-gray-400 mb-3 uppercase tracking-wide">Stress Level</div>
                      <div className="flex items-center justify-center gap-4">
                        <div className="text-4xl font-bold text-red-400">
                          {stressLevel.toFixed(1)}%
                        </div>
                        <div className="w-40 bg-gray-800 rounded-full h-6 overflow-hidden border-2 border-gray-700">
                          <div
                            className={`h-full transition-all duration-1000 ${
                              stressLevel > 60 ? "bg-gradient-to-r from-red-600 to-red-400" :
                              stressLevel > 30 ? "bg-gradient-to-r from-orange-500 to-orange-400" : 
                    ...
                    <div>
                      <div className="text-sm text-gray-400 mb-3 uppercase tracking-wide">Relaxation Level</div>
                      <div className="flex items-center justify-center gap-4">
                        <div className="text-4xl font-bold text-green-400">
                          {relaxationLevel.toFixed(1)}%
                        </div>
                        <div className="w-40 bg-gray-800 rounded-full h-6 overflow-hidden border-2 border-gray-700">
                          <div
                            className={`h-full transition-all duration-1000 ${
                              relaxationLevel > 60 ? "bg-gradient-to-r from-green-600 to-green-400" :
                              relaxationLevel > 30 ? "bg-gradient-to-r from-blue-500 to-blue-400" : 
                              "bg-gradient-to-r from-gray-500 to-gray-400"
                            }`}
                            style={{ width: `${relaxationLevel}%` }}
                          />
                        </div>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
                ...
                {history.map((h, i) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-300 mb-1">
                      Stress: <span className="text-red-400 font-semibold">{h.stressLevel.toFixed(1)}%</span>
                      {" | "}
                      Relaxation: <span className="text-green-400 font-semibold">{h.relaxationLevel.toFixed(1)}%</span>
                    </div>
          ...
          <Card title="ℹ️ Quick Guide">
            <div className="text-sm space-y-3 text-gray-300">
              ...
              <div className="bg-gray-800 p-3 rounded">
                <div className="font-semibold text-white mb-1">5. Mental States</div>
                <div className="text-xs mt-1 space-y-1">
                  <div>🟢 Baseline - Normal resting state</div>
                  <div>🟢 Reading - Focused attention</div>
                  <div>🟢 Relaxation - Calm state</div>
                  <div>🔴 Cognitive Stress - Mental strain</div>
                  <div>🔴 Social Stress - Social pressure</div>
                </div>
              </div>
            </div>
          </Card>
