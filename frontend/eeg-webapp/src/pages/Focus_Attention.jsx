...
// ─────────────────────────────────────────────────────────────────
function CsvUploadCard({ fs }) {
  const [csvFile, setCsvFile] = useState(null);
  const [csvColumn, setCsvColumn] = useState("");
  const [csvStatus, setCsvStatus] = useState("idle"); // idle | uploading | success | error
  ...
  const handleUpload = async () => {
    ...
    setCsvError("");
    setCsvResult(null);
    setCsvProgress("Uploading and processing…");

    try {
      const formData = new FormData();
      formData.append("file", csvFile);
      formData.append("fs", String(fs));
      if (csvColumn.trim()) formData.append("column", csvColumn.trim());

      setCsvProgress("Sending to model…");

      const response = await fetch("http://127.0.0.1:8001/predict-csv", {
        method: "POST",
        body: formData,
      });
      ...
      if (!response.ok || data.error) {
        setCsvStatus("error");
        setCsvError(data.error || `Server error ${response.status}`);
        setCsvProgress("");
        return;
      }
      ...
      setCsvProgress("");
      console.log("✅ CSV prediction result:", data);
    } catch (err) {
      setCsvStatus("error");
      setCsvError("Cannot reach Flask API. Make sure python app.py is running.");
      setCsvProgress("");
    }
  };
  ...
  const stateColor = (state) => {
    ...
    if (s.includes("focus")) return "#10B981";
    if (s.includes("distract")) return "#EF4444";
    if (s.includes("baseline")) return "#3B82F6";
    return "#9CA3AF";
  };

  return (
    <Card title="📂 Predict from CSV File">
      {/* Drop Zone */}
      <div
        onDrop={handleDrop}
        ...
        style={{
          border: `2px dashed ${csvFile ? "#10B981" : "#374151"}`,
          borderRadius: 10,
          padding: "20px 16px",
          textAlign: "center",
          cursor: "pointer",
          background: csvFile ? "rgba(16,185,129,0.06)" : "rgba(55,65,81,0.2)",
          transition: "all 0.2s",
          marginBottom: 14,
        }}
      >
        ...
        {csvFile ? (
          ...
          <div>
            ...
            <div style={{ color: "#9CA3AF", fontSize: 13 }}>
              Drop a <span style={{ color: "#60A5FA" }}>.csv</span> file here, or click to browse
            </div>
            <div style={{ color: "#4B5563", fontSize: 11, marginTop: 4 }}>
              Single-column or multi-column EEG data
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ color: "#9CA3AF", fontSize: 12, display: "block", marginBottom: 4 }}>
          Column name <span style={{ color: "#4B5563" }}>(optional — leave blank to auto-detect)</span>
        </label>
        <input
          type="text"
          value={csvColumn}
          onChange={(e) => setCsvColumn(e.target.value)}
          placeholder='e.g. "eeg" or "value"'
          style={{
            width: "100%",
            background: "#1F2937",
      ...
      <div style={{
        ...
        <span style={{ color: "#9CA3AF", fontWeight: 600 }}>Accepted formats:</span>
        <br />
        • Single column, no header: one EEG value per row
        <br />
        • With header row: <code style={{ color: "#60A5FA" }}>eeg</code>, <code style={{ color: "#60A5FA" }}>value</code>, etc.
        <br />
        • Multi-column: specify column name above, or first numeric column is used
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        <button
          ...
          style={{
            ...
            borderRadius: 8,
            border: "none",
            cursor: !csvFile || csvStatus === "uploading" ? "not-allowed" : "pointer",
            background: !csvFile || csvStatus === "uploading"
              ? "#374151"
              : "linear-gradient(135deg,#1D4ED8,#3B82F6)",
            color: "#fff",
            fontWeight: 700,
            fontSize: 14,
            opacity: !csvFile || csvStatus === "uploading" ? 0.6 : 1,
            transition: "all 0.2s",
          }}
        >
          {csvStatus === "uploading" ? "⏳ Processing…" : "⚡ Run Prediction"}
        </button>
        {(csvFile || csvResult) && (
          <button
            onClick={handleClear}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ color: "#6B7280", fontSize: 11 }}>
              {csvResult.windows_processed} window{csvResult.windows_processed !== 1 ? "s" : ""} · {csvResult.duration_s}s
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
            <div style={{
              ...
              <div style={{ color: "#6B7280", fontSize: 10, marginBottom: 4, textTransform: "uppercase" }}>
                Dominant State
              </div>
              <div style={{
                fontWeight: 800, fontSize: 18,
                color: stateColor(csvResult.prediction),
              }}>
                {LABEL_NAMES[csvResult.prediction] || csvResult.prediction}
              </div>
            </div>
            <div style={{
              background: "#1F2937", borderRadius: 8, padding: "10px 12px",
              ...
              <div style={{ fontWeight: 800, fontSize: 18, color: "#60A5FA" }}>
                {csvResult.focus_level.toFixed(1)}%
              </div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{
              ...
              <div style={{
                height: "100%",
                width: `${csvResult.focus_level}%`,
                borderRadius: 5,
                background: csvResult.focus_level > 60
                  ? "linear-gradient(90deg,#059669,#10B981)"
                  : csvResult.focus_level > 30
          ...
          {/* State breakdown */}
          {csvResult.state_counts && Object.keys(csvResult.state_counts).length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ color: "#6B7280", fontSize: 11, marginBottom: 6, textTransform: "uppercase" }}>
                State Distribution
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {Object.entries(csvResult.state_counts)
                  .sort(([, a], [, b]) => b - a)
                  .map(([state, count]) => {
                    const pct = Math.round((count / csvResult.windows_processed) * 100);
                    const label = LABEL_NAMES[state] || `State ${state}`;
                    return (
                      <div key={state} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{
                          width: 70, fontSize: 11, color: stateColor(label),
          ...
          {/* Probabilities */}
          {csvResult.probabilities && Object.keys(csvResult.probabilities).length > 0 && (
            <div>
              <div style={{ color: "#6B7280", fontSize: 11, marginBottom: 6, textTransform: "uppercase" }}>
                Avg Classification Confidence
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                {Object.entries(csvResult.probabilities)
                  .sort(([, a], [, b]) => b - a)
                  .map(([cls, prob]) => (
                    <div key={cls} style={{
                      display: "flex", justifyContent: "space-between",
                      background: "#1F2937", borderRadius: 6, padding: "5px 10px",
                      fontSize: 12,
                    }}>
                      <span style={{ color: "#D1D5DB" }}>
                        {LABEL_NAMES[cls] || `State ${cls}`}
                      </span>
          ...
          <div style={{
            ...
            <span>📄 {csvResult.filename}</span>
            <span>{csvResult.total_samples?.toLocaleString()} samples · {csvResult.sample_rate}Hz</span>
          </div>
        </div>
      )}
    </Card>
  ...
  const disconnectSerial = async () => {
    try {
      ...
      if (readerRef.current) {
        try { await readerRef.current.cancel().catch(() => {}); } catch (e) {}
        ...
        <div className="space-y-6">
          ...
          <Card title="ℹ️ Quick Guide">
            <div className="text-sm space-y-3 text-gray-300">
              ...
              <div className="bg-gray-800 p-3 rounded">
                <div className="font-semibold text-white mb-1">4. Or Upload CSV</div>
                <div>Skip the device — upload a .csv file for offline prediction</div>
              </div>
              <div className="bg-gray-800 p-3 rounded">
                <div className="font-semibold text-white mb-1">5. View Results</div>
                <div>Real-time predictions every 3s, or instant CSV results</div>
              </div>
            </div>
          </Card>
        </div>
